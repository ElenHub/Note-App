...
//             },
//             prepare: ({ title, details, color, fontColor, date }: Omit<NoteType , 'id'>) => {
//                 return {
//                     payload: {
//                         id: uuidv4(),
//                         title,
...
//             state.notes = action.payload; // Устанавливаем заметки при загрузке из локального хранилища
//         },
//     },
// });
        ...
        addNote: {
            reducer: (state, action: PayloadAction<NoteType>) => {
                if (!Array.isArray(state.notes)) {
                    state.notes = []; // Обработка случая, когда notes не массив
                }
                state.notes.push(action.payload);
            },
            prepare: ({ title, details, color, fontColor, date }: Omit<NoteType , 'id'>) => {
